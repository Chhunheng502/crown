<template>
    <jet-dropdown align="center" width="96">
        <template #trigger>
            <div class="text-sm text-gray-500 cursor-pointer" @click="markNotifications">
                <span v-if="responsive"> <i class="text-black fas fa-bell fa-lg"></i> </span>
                <span v-else> Notifications </span>
                <span v-show="numOfUnread !== 0" class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 bg-red-600 rounded-full">{{ numOfUnread }}</span>
            </div>
        </template>

        <template #content>
            <div v-if="notifications.length !== 0" class="h-64 overflow-auto">
                <div
                ...
                >
                    <div class="flex space-x-2">
                        <crown-image :src="notification.profile_photo_path ?? notification.data.profile_photo_path" class="w-14 h-14" />
                        <div class="leading-5">
                            <h1>
