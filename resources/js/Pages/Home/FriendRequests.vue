  ...
  <div :class="responsive ? 'space-y-2 h-64 overflow-auto' : 'space-y-2'">
    <div>
      <h1 class="text-xl font-bold">
        <i class="fas fa-user-friends" /> Friend Requests
      </h1>
    </div>
    <div class="space-y-2">
      <div
        ...
        :key="request.id"
        class="px-2 py-3 rounded-lg shadow-lg bg-gray-300"
      >
        <div class="flex space-x-2">
          ...
          <crown-profile-image
            :src="request.requester.profile_photo_path"
            class="w-14 h-14"
          />
          <div class="space-y-2">
            <div class="leading-5">
...
export default defineComponent({
  ...
  methods: {
    ...
    acceptRequest (id) {
      axios.post('/acceptFriendRequest', { id }).then(response => {
    ...
    deleteRequest (id) {
      axios.post('/deleteFriendRequest', { id }).then(response => {
        console.log(response.data)
