<template>
  ...
  <div>
    <div
      ...
      @mouseover="showUploadCover = true"
      @mouseleave="showUploadCover = false"
    >
      <!-- will need to adjust the cover -->
      <crown-image
        :src="friend.detail.cover"
    ...
    <div class="flex justify-between items-center px-6 py-3 bg-gray-300">
      <div class="flex items-center space-x-2">
        <div
          class="relative w-28 h-28 rounded-full overflow-hidden"
          @mouseover="showUploadProfile = true"
          @mouseleave="showUploadProfile = false"
        >
          <crown-image
            :src="friend.profile_photo_path"
            class="w-full h-full object-cover"
            alt="Profile image"
          />
          <transition name="fade">
      ...
      >
        <div>
          <crown-button
            v-if="!isFriend"
            class="px-2 py-1 bg-blue-400"
            @click="addFriend"
            v-text="!pending ? 'Add Friend' : 'Pending Request'"
          />
          <crown-button
            v-else
            class="px-2 py-1 bg-red-400"
            @click="unfriend"
          >
            Unfriend
          </crown-button>
  ...
  <section-layout>
    <template #section-left>
      <div class="space-y-2">
        ...
        <crown-button
          v-show="user.id === friend.id"
          class="w-full px-2 py-1 bg-gray-400"
          @click="showEditModal = true"
        >
          Edit Detail
        </crown-button>
        <edit-modal
          :user_detail="friend.detail"
          :show="showEditModal"
    ...
    <template #section-middle>
      <div class="my-3 space-y-5">
        <div
          v-for="post in getSortedPosts"
          :key="post.id"
        >
          <post-card
            v-if="post.post_id == null"
            :user="friend"
            :post="post"
          />
          <share-card
