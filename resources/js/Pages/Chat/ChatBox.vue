    ...
    <section-layout>
      <template #section-left>
        <chat-list
          :from_user="from_user"
          :chats="chats"
          @chatClicked="changeChat"
        />
      </template>
      <template #section-middle>
        <div
          v-show="hasExistingChat()"
          class="flex-1 p:2 sm:p-6 justify-between flex flex-col h-screen border-2 border-gray-300 rounded-lg"
        >
          <chat-upper-bar
