    ...
    <div class="flex space-x-2">
        ...
        <div v-show="!viewEdit" class="w-full">
            <div class="flex items-center space-x-2">
                ...
                <popover v-show="user.id === comment.user.id" popoverClass="rounded-full w-8 h-8 hover:bg-gray-300">
                    ...
                    <template #items>
                        ...
                        <div>
                            <button type="button" class="w-full px-4 py-2 hover:bg-gray-100" @click="viewEdit = true"> Edit </button>
                        </div>
                        <div>
                            <button type="button" class="w-full px-4 py-2 hover:bg-gray-100" @click="deleteUserComment"> Delete </button>
                        </div>
                    </template>
                </popover>
            </div>
            <div class="flex ml-2 space-x-2 text-sm">
                ...
                <button class="hover:underline" @click="replyClicked = true"> Reply </button>
                <p> {{ comment.created_at }} </p>
            </div>
            <form v-show="replyClicked && (user.id !== comment.user.id)" class="flex mt-2 space-x-2 items-center" @submit.prevent="replyUserComment">
                <crown-profile-image :src="user.profile_photo_path" class="w-8 h-8" />
                <crown-input
            ...
            <div v-show="viewReply" class="mt-3 space-y-2">
                <replied-comments @triggerReply="replyOtherComment" @triggerDelete="deleteOtherComment" v-for="comment in repliedComments" :key="comment.id" :user="user" :comment="comment"> </replied-comments>
            </div>
            <button v-show="showRepliedCmtBtn()" type="button" class="ml-2 text-sm hover:underline" @click="viewReply = true"> <i class="fas fa-reply rotate-180 mr-2"></i> {{ repliedComments.length }} Reply </button>
        </div>
        <form v-show="viewEdit">
            <crown-textarea
                type="text" class="w-full" rows="5" placeholder="Write something..."
                @keyup.enter="editComment" v-model="content"
            />
        </form>
    </div>
</template>
